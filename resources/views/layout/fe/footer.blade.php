<!-- Footer -->
<footer class="text-center text-lg-start bg-primary py-3 text-white">

    <!-- Section: Social media -->

    <!-- Section: Links  -->
    <section class="">
        <div class="container text-center text-md-start mt-5">
            <!-- Grid row -->
            <div class="row mt-3">
                <!-- Grid column -->
                <div class="col-md-3 col-lg-4 col-xl-3 mx-auto mb-4">
                    <!-- Content -->
                    <h6 class="text-uppercase fw-bold mb-4">
                        <i class="fa-solid fa-school me-3"></i>SMAN 7 Mataram
                    </h6>
                    <p>
                        Didirikan pada tahun 2003. Sekolah ini mendapat prestasi berturut – turut dalam ajang
                        sekolah sehat.
                    </p>
                </div>
                <!-- Grid column -->

                <!-- Grid column -->
                <div class="col-md-3 col-lg-2 col-xl-2 mx-auto mb-4">
                    <!-- Links -->
                    <h6 class="text-uppercase fw-bold mb-4">
                        Sosial Media
                    </h6>
                    <p>
                        <i class="fa-brands fa-instagram"></i>
                        <a href="#!" class="text-reset">Instagram

                        </a>
                    </p>
                    <p>
                        <i class="fa-brands fa-facebook"></i>
                        <a href="#!" class="text-reset">Facebook</a>
                    </p>
                    <p>
                        <i class="fa-brands fa-twitter"></i>
                        <a href="#!" class="text-reset">Twitter</a>
                    </p>
                    <p>
                        <i class="fa-brands fa-youtube"></i>
                        <a href="#!" class="text-reset">YouTube</a>
                    </p>
                </div>
                <!-- Grid column -->

                <!-- Grid column -->
                <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mb-md-0 mb-4">
                    <!-- Links -->
                    <h6 class="text-uppercase fw-bold mb-4">
                        Hubungi Kami
                    </h6>
                    <p>
                        <i class="fa-solid fa-location-dot me-3"></i>
                        Alamat: Jalan Adisucipto No. 69 <br>&emsp;&emsp;Ampenan, Mataram
                    </p>
                    <p>
                        <i class="bi bi-envelope me-3"></i>
                        sman7mataram@yahoo.co.id
                    </p>
                    <p><i class="bi bi-phone me-3"></i> (0370) 6162545</p>
                </div>
                <!-- Grid column -->
            </div>
            <!-- Grid row -->
        </div>
    </section>
    <!-- Section: Links  -->

    <!-- Copyright -->
    <div class="text-center py-4" style="background-color: rgba(0, 0, 0, 0.05);">
        Copyright @ 2022 sman7mataram | Design by Mataram Web
    </div>
    <!-- Copyright -->
</footer>
<!-- Footer -->
